...
- **Robust Fallback Implementation**: Updated `run_bts_moga()` in [`src/nkm/moga.py`](file:///home/cspark/Work/projects/nkm/src/nkm/moga.py) to fall back to `least_infeasible_x` / `least_infeasible_f` when `pareto_x` is empty, populating representative solutions even on short test runs.
- **Test Alignment**: Updated [`tests/test_moga_pareto.py`](file:///home/cspark/Work/projects/nkm/tests/test_moga_pareto.py) assertions to check `len(res.representative_solutions) > 0`.
- **Verification**:
