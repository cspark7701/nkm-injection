...
- **Fix Applied**: Updated `validate_publication_manifest()` in [`src/nkm/results_schema.py`](file:///home/cspark/Work/projects/nkm-injection/src/nkm/results_schema.py) to automatically compute and write `results/baseline/protected_files_manifest.json` on the fly when `create_if_missing` is `True` and the manifest file is missing.
- **Verification**:
